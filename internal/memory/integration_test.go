...
func TestRemember_HappyPath(t *testing.T) {
	...
	res, err := s.Remember(ctx, RememberPayload{
		Summary: "Widget upgraded to v0.7. Strict flag now required for builds.",
		Details: "Hit during migration on 2026-04-01.",
		Kind:    KindProcedure,
		Tags:    []string{"widget", "upgrade"},
		Author:  "test-agent",
		Project: "homelab",
		Entities: []EntityDecl{
			{Name: "Widget", Type: EntityTool, Aliases: []string{"widget-cli"}},
			{Name: "v0.7", Type: EntityConcept},
		},
		Edges: []EdgeDecl{
			{Subject: "Widget", SubjectType: EntityTool,
...
func TestRemember_RejectsUndeclaredEdgeEndpoints(t *testing.T) {
	...
	_, err := s.Remember(ctx, RememberPayload{
		Summary: "Widget version v0.7", Kind: KindFact, Author: "claude",
		Entities: []EntityDecl{
			{Name: "Widget", Type: EntityTool}, // v0.7 NOT declared
		},
		Edges: []EdgeDecl{
			{Subject: "Widget", SubjectType: EntityTool,
				Predicate: "version_is",
...
func TestRemember_ExclusivePredicateSupersession(t *testing.T) {
	...
	// Widget version_is v0.6 (initial)
	_, err := s.Remember(ctx, RememberPayload{
		Summary: "Widget at v0.6.", Kind: KindFact, Author: "claude",
		Entities: []EntityDecl{
			{Name: "Widget", Type: EntityTool},
			{Name: "v0.6", Type: EntityConcept},
		},
		Edges: []EdgeDecl{{Subject: "Widget", SubjectType: EntityTool,
	...
	// Now Widget version_is v0.7 — should supersede the v0.6 edge.
	r2, err := s.Remember(ctx, RememberPayload{
		Summary: "Widget upgraded to v0.7.", Kind: KindFact, Author: "claude",
		Entities: []EntityDecl{
			{Name: "Widget", Type: EntityTool},
			{Name: "v0.7", Type: EntityConcept},
		},
...
func TestEditHandNotes_AppendThenReplace(t *testing.T) {
	...
	_, err := s.Remember(ctx, RememberPayload{
		Summary: "Seeding Widget entity.", Kind: KindFact, Author: "claude",
	...
	if err := s.EditHandNotes(ctx, "Widget", EntityTool, "First line.", NotesAppend, "claude"); err != nil {
		t.Fatal(err)
	}
	if err := s.EditHandNotes(ctx, "Widget", EntityTool, "Second line.", NotesAppend, "alice"); err != nil {
		t.Fatal(err)
	}
	...
	if err := s.EditHandNotes(ctx, "Widget", EntityTool, "Replaced.", NotesReplace, "alice"); err != nil {
		t.Fatal(err)
	}
...
func TestForget_SoftDeletesAndExcludesFromRecall(t *testing.T) {
	...
	ctx := context.Background()
	r, err := s.Remember(ctx, RememberPayload{
		Summary: "Widget at v0.5.", Kind: KindFact, Author: "claude",
		Entities: []EntityDecl{{Name: "Widget", Type: EntityTool}},
	})
	if err != nil {
		t.Fatal(err)
