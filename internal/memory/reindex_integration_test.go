...
func TestReindexEmbeddings_Backfill(t *testing.T) {
	...
	if _, err := plain.Remember(ctx, RememberPayload{
		Summary: "Jason vented at a stateless AI at 3am because it forgot him every session.",
		Kind:    KindIdentity, Author: "claude",
	...
	hits, err := emb.Recall(ctx, RecallQuery{Query: "screaming at forgetful chatbots in the middle of the night", K: 5})
	if err != nil {
		t.Fatalf("recall: %v", err)
	}
	found := false
	for _, m := range hits.Memories {
		if strings.Contains(m.Summary, "vented at a stateless AI") {
			found = true
			break
		}
	}
